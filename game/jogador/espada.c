...
void ataqueEspada(Jogo* jogo) {
    if (jogo->jogador.instantesEspada > 0 && jogo->jogador.temEspada == 1) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            for (int j = 0; j < 3; ++j) {
                if (jogo->jogador.tilesAtaque[j].y == jogo->monstro[i].pos.y && jogo->jogador.tilesAtaque[j].x == jogo->monstro[i].pos.x) {
                    jogo->monstro[i].vivo = 0;
                    jogo->jogador.pontuacaoTotal += jogo->monstro[i].pontuacao;
                }
            }
            }
        }
